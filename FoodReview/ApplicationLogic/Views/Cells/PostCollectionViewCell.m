...
- (UIButton *)likeButton {
    if (!_likeButton) {
        ...
        _likeButton.tintColor = UIColor.steelColor;
        ...
        [_likeButton setImage:[UIImage systemImageNamed:@"eye" withConfiguration:config] forState: UIControlStateNormal];
...
-(void)configCellWithReview:(Review *)review {
    [self.artwork sd_setImageWithURL:[NSURL URLWithString:review.images.firstObject] placeholderImage:[Common imageFromColor:UIColor.grayColor]];
    [self.avatarImage sd_setImageWithURL:[NSURL URLWithString:review.userAvatar] placeholderImage:[UIImage imageNamed:@"avatar-default"]];
    self.titleLabel.text = review.title;
    self.nameLabel.text = review.userName;
    [self.likeButton setTitle:[Common suffixNumber:[NSNumber numberWithInteger:review.totalViews]] forState:UIControlStateNormal];
}
