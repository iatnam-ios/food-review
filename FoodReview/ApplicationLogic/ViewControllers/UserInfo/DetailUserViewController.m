...
@interface DetailUserViewController ()<JXPagerViewDelegate, JXCategoryViewDelegate, UserTableHeaderViewDelegate>
        ...
        [followerQuery addSnapshotListener:^(FIRQuerySnapshot * _Nullable snapshot, NSError * _Nullable error) {
            if (error) {
                ...
                if (self.firUser) {
                    ...
                    self.userHeaderView.followButton.selected = NO;
                }
            }
        }];
...
- (void)didPressFollowButton:(UIButton *)sender {
    ...
    if (user) {
        if (!sender.isSelected) {
            Relationship *rela = [[Relationship alloc] initWithFollowerId:user.uid followedId:self.userId createdDate:[NSDate date]];
            ...
            NSString *relaId = [NSString stringWithFormat:@"%@_%@", user.uid, self.userId];
